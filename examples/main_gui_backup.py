...
import shutil
import socket
from examples.vehicles import VirtualDrone
...
if __name__ == '__main__':
    ...
    while len(clients) < 2:  # Wait for at least one client to connect
        conn, addr = s.accept()
    ...
    survivor_index = 0
    # print(f'Survivor images: {survivor_images}')
    # assert False
    ...
    game_mgr = gui_panel.GameMgr(drones)
    game_mgr.update()

    # # Create user GUIs and pipes
    # n_users = 2
    # q_image = [mp.Queue(maxsize=1) for _ in range(n_users)]
    # q_decision = [mp.Queue(maxsize=1) for _ in range(n_users)]
    # user_processes = [mp.Process(target=run_user_gui, 
    #                             args=(q_image[i], q_decision[i])) for i in range(n_users)]
                ...
                if stay_time[idx] > stay_duration:
                    if target_index[idx] < len(drone_trajectory[idx]) - 1:
                        ...
                        if not game_mgr.victim_detected[idx]:
                            ...
                            clients[idx][0].sendall(str(survivor_images[survivor_index]).encode())
                            survivor_index += 1
                        ...
                        if game_mgr.target_decided:
                            try:
                                data = clients[idx][0].recv(1024).decode()
                                if data:
                                    game_mgr.victim_clicked[idx] = 1 if data == 'accept' else 2
                                    data = None
