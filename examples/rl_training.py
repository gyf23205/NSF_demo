...
# ---- Timed event scheduler (script-side; no core changes) -------------------
class EventScheduler:
    ...
    def _choose_tid_for_fire(self, ws):
        # pick a target near a random vehicle; fall back to any
        try:
        ...
        if all_agents:
            ...
            if locs:
                ...
                # prefer ones not already high priority
                choices = [tid for tid in cand if ws.get_target_priority(tid) < 2]
                if not choices:
                    choices = list(range(len(locs)))
                return int(self.rng.choice(choices) if self.rng is not None else np.random.choice(choices))
        # fallback
        n = len(getattr(ws, "target_locations", []))
        return int(self.rng.integers(n) if (self.rng is not None and n > 0) else (0 if n > 0 else 0))

    def tick(self, t_now: float, ws, labeler):
        # FIRE -> raise priority at scheduled times, then emit p_firemsg
    ...
    def on_completed(self, ap: str, labeler):
        # When a verify finishes, choose found/notfound gate (like your old code)
