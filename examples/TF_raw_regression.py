...
class TransformerRawRegressor(pl.LightningModule):
    def __init__(self, config, optim_cfg, pre_process):
        ...
        self.num_outputs = config.get("num_classes", 1)  # For regression, this is output dimensions
        ...
        # Use linear layer for regression output
        self.regressor = nn.Linear(self.d_model, self.num_outputs)
    ...
    def forward(self, a1, a2, pre_process=None):
        ...
        output = self.regressor(pooled)  # [B, num_outputs]
        
        # For single output regression, squeeze the last dimension
    ...
    def training_step(self, batch, batch_idx):
        t1, t2, labels = batch
        output = self.forward(t1, t2, self.pre_process)
